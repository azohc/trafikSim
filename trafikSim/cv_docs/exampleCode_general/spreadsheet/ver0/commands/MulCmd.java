...
import tp.examples.spreadsheet.ver0.SPCellMul;
...
public class MulCmd implements Command {
	...
	private MulCmd(int x, int y, int x1, int y1, int x2, int y2 ) {	
		this.x = x;
		this.y = y;
		this.x1 = x1;
		this.y1 = y1;
	...
	public void execute(SpreadSheet s, Controller c) {
		s.setCell(x, y, new SPCellMul(x1, y1, x2, y2));
	}
	...
	public Command parse(String line) {
		...
		try {
			...
			return new MulCmd(x,y,x1,y1,x2,y2);
